...
function get_data_forte_prime_plus() {
    $handle = fopen('../data/prime_mieux_1200000_Forte.csv', 'r');
    ...
    $points = [];
    foreach ($scala as $entry) {
        if (isset($entry['Nodes']) && is_numeric($entry['Nodes'])) {
            $processus[] = (int) $entry['Nodes'];
        }
    }
